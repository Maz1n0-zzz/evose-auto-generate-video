...
## 🖥️ Evose Studio — Giao diện Web

`app.html` là UI tĩnh chạy thẳng trong trình duyệt — không cần server, không cần cài thêm gì.
...
Hoặc double-click `app.html` trong Finder / File Explorer.

**Tính năng:**

| Tính năng | Mô tả |
|-----------|-------|
| **URL Input** | Dán link bài báo — AI đọc và phân tích nội dung |
...
| **Xuất Video** | Hiển thị câu lệnh CLI để copy và chạy trong terminal |

**Quy trình:**

1. Dán URL bài báo vào thanh trên cùng
2. Chọn phong cách template từ các frame card
3. Bấm **Xuất Video** — câu lệnh CLI xuất hiện trong ô log
4. Copy câu lệnh → chạy trong terminal → chờ lấy `output/<slug>/video.mp4`

> `app.html` chỉ là công cụ lập kế hoạch — render **không chạy trong trình duyệt**.
> Sau khi bấm Xuất Video, paste câu lệnh vào terminal:
> ```bash
...
Đây là cách repo này được xây dựng và vận hành — Claude Code đóng vai AI, pipeline lo phần sản xuất.
...
Hoặc tải tại [claude.ai/download](https://claude.ai/download).

### 2. Clone repo và mở bằng Claude Code
...
Skill `/create-template-video` tự load khi bạn mở thư mục dự án.

### 3. Tạo video

```text
/create-template-video https://link-bai-bao-cua-ban
```

Claude sẽ tự động:

1. Đọc và tóm tắt nội dung bài báo
2. Viết `output/<slug>/script.json` — chọn template từng scene + viết voiceover tiếng Việt chuẩn TTS
3. Chạy `npm run pipeline` tự động
4. Trả về đường dẫn tới `video.mp4`, `voice.mp3`, `script.txt`

> Toàn bộ quy tắc biên soạn (map template, viết số ra chữ cho TTS, brand fields Evose) có trong
...
[![Xem Demo](./assets/demo-frame.jpg)](https://evose.ai/)
